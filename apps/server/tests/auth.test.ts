...
if (!runIntegrationTests) {
  test(
    "auth integration tests are skipped without RUN_SERVER_INTEGRATION_TESTS=1",
  );
} else {
  before(async () => {
  ...
  test(
    "auth sign-up endpoint creates a session for a new renter",
    { concurrency: false },
    async () => {
      ...
      assert.equal(sessionBody.user.role, harness.userRole.RENTER);

      const renterProfile = await harness.prisma.renter.findUnique({
        where: {
  ...
  test(
    "auth sign-in and sign-out endpoints manage session cookies",
    { concurrency: false },
    async () => {
      ...
      const afterSignOut = await harness.request(
        "/api/auth/get-session",
        {},
        cookieJar,
      );
